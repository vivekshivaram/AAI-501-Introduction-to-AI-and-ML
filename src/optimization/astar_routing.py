...
class AStarRouting(RoutePlanner):
    ...
    """
    def __init__(self, graph: Graph, heuristic: Heuristic, delay_map: DelayMap | None = None) -> None:
        self._graph = graph
    ...
    def shortest_path( self, source: int, destination: int) -> Path:
        ...
        return self._search(self._graph.get_node(source), self._graph.get_node(destination))
        

    def _search( self, start: Node, goal: Node) -> Path:
        open_set: list[tuple[float, int, int]] = []
        counter = count()
        heapq.heappush(open_set, (0.0, next(counter), start.id))
        #
        # node_id ->
        # (
        #     parent_node,
        ...
        while open_set:
            ...
            if current_id == goal.id:
                return self._build_path(came_from, start.id, goal.id, g_score[goal.id])
            ...
            for edge in self._graph.outgoing_edges(current.id):
                ...
                cost = edge.travel_time + self._edge_penalty(edge)
                tentative = g_score[current.id]+ cost
                if tentative >= g_score.get(neighbour, float("inf")):
                    continue

                came_from[neighbour] = (current.id,  edge)
                g_score[neighbour] = tentative
                heuristic = self._heuristic.estimate(self._graph, self._graph.get_node(neighbour), goal)
                heapq.heappush(open_set, (tentative + heuristic, next(counter), neighbour))
                
        raise RouteNotFoundError(f"No route between {start.id} and {goal.id}")
        

    def _build_path(self, came_from: dict[int, tuple[int, Edge]], source: int,  destination: int, total_cost: float) -> Path:
        ...
        node_ids.reverse()
        edges.reverse()
        nodes = [ self._graph.get_node(node) for node in node_ids ]
        total_distance = sum(edge.length for edge in edges)
        total_travel_time = sum(edge.travel_time for edge in edges)
        return Path(nodes=nodes, edges=edges, total_distance=total_distance, total_travel_time=total_travel_time, total_cost=total_cost)
        

    def _edge_penalty(self, edge: Edge ) -> float:
