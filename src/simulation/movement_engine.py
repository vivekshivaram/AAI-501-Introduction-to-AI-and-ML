...
class MovementEngine:
    ...
    def move_vehicle(self, vehicle: Vehicle, position: VehiclePosition) -> MovementResult:
        ...
        position.current_edge = (position.current_node, next_node)
    ...
    def move_all(self, vehicles: list[Vehicle], positions: dict[str, VehiclePosition]) -> dict[str, MovementResult]:
        results = {}
        for vehicle in vehicles:
            if vehicle.vehicle_id not in positions:
                continue
            results[vehicle.vehicle_id] = self.move_vehicle(vehicle, positions[vehicle.vehicle_id])
